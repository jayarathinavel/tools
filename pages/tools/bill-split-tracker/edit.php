<?php
    ...
    initializePage("Edit Bill", "main", $_SERVER['REQUEST_URI']);
    includePhpFileFromRoot($rootPath, '/pages/tools/bill-split-tracker/bill-split-utils.php');
    $id = $_GET['id'];
    $bill = executeQuery("SELECT * FROM bill_split WHERE id=$id")->fetch_assoc();
    $userId = billSplitUser();
    $persons = fetchPersonsFromBillSplitBook($bill['bill_split_book_id']);
...
<div class="container">
    ...
    <form action="" method="post" id="billForm">
        ...
        <div class="form-group">
            ...
            <input type="text" id="bill_name" name="bill_name" class="form-control" value="<?php echo htmlspecialchars($bill['bill_name']); ?>" required>
        </div>
        ...
        <div class="form-group">
            ...
            <select id="paid_by" name="paid_by" class="form-control" required>
                <?php foreach ($persons as $person): ?>
                    <option value="<?php echo htmlspecialchars($person); ?>" <?php echo ($person == $bill['paid_by']) ? 'selected' : ''; ?>>
        ...
        <div class="form-group">
            ...
            <input type="date" id="date" name="date" class="form-control" value="<?php echo $bill['date']; ?>" required>
        </div>
        
        <div class="form-group">
            <label>Split Type:</label>
            <div class="form-check">
                <input class="form-check-input" type="radio" name="split_type" id="equal_split" value="equal" <?php echo ($bill['split_type'] == 'equal') ? 'checked' : ''; ?> onchange="toggleSplitType()">
                <label class="form-check-label" for="equal_split">
                    Equal Split
                </label>
            </div>
            <div class="form-check">
                <input class="form-check-input" type="radio" name="split_type" id="custom_split" value="custom" <?php echo ($bill['split_type'] == 'custom') ? 'checked' : ''; ?> onchange="toggleSplitType()">
        ...
        <div id="custom_splits" class="form-group" style="display: none;">
            ...
            <div class="row mb-2">
                ...
                <div class="col-md-9">
                    <input type="number" step="0.01" id="split_<?php echo $index; ?>" name="splits[<?php echo htmlspecialchars($person); ?>]" class="form-control custom-split-input" value="<?php echo isset($splits[$person]) ? $splits[$person] : '0'; ?>" onchange="updateCustomTotal()">
                </div>
            </div>
            <?php endforeach; ?>
            <div class="mt-2">
        ...
        <input type="submit" value="Update Bill" class="btn btn-primary mt-2" id="submit_btn">
