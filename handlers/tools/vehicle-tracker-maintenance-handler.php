...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['id'])) { // Update record
        $id = $_POST['id'];
        $conn->query("UPDATE vt_maintenance  SET date='$date', vehicle_id='$vehicle_id', odometer_start='$odometer_start', odometer_due='$odometer_due', description='$description' WHERE id=$id");
        setSuccessOrFailureMessage("success", "Maintenance record updated successfully.");
    } else { // Add new record
        $vehicle_id = findVehicleForUser($userId);
        $conn->query("INSERT INTO vt_maintenance  (vehicle_id, date, odometer_start, odometer_due, description) VALUES ('$vehicle_id', '$date', '$odometer_start', '$odometer_due', '$description')");
